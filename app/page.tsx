...
export default function Home() {
  return <main id="top">
    <header className="issue-nav" aria-label="Primary navigation"><a className="issue-mark" href="#top" aria-label="Akshat Kadam, home">AK<span>!</span></a><p className="issue-number">PERSONAL ARCHIVE / ISSUE <AgeIssue /></p><nav><a href="#origin">My Story</a><a href="#work">9–5</a><a href="#now">5–9</a><a href="/projects">Projects</a><a href="/bookshelf">Bookshelf</a></nav></header>
    <CoverReader />
    ...
    <section className="arc-section" id="work" aria-labelledby="work-title"><div className="section-ribbon"><span>9–5 · HUSTLE AND BUSTLE</span><span>9–5 · HUSTLE AND BUSTLE</span><span>9–5 · HUSTLE AND BUSTLE</span></div><div className="arc-intro"><p className="chapter-label">9–5 / HUSTLE AND BUSTLE</p><h2 id="work-title">Three things<br /><em>in motion.</em></h2><p>Companies and products are not trophies here. They are ongoing storylines: problems worth staying with, people worth building for, and experiments still unfolding.</p></div><div className="venture-panels">{ventures.map((venture, index) => <article className={`venture-panel ${venture.tone}`} key={venture.name}><div className="venture-top"><span>{venture.tag}</span><b>0{index + 1}</b></div><h3>{venture.name}</h3><p>{venture.copy}</p><span className="venture-action">Open this arc <i aria-hidden="true">↗</i></span></article>)}</div></section>
